...
const Manager = () => {
    ...
    const getpasswords = async () => {
        let req = await fetch('http://34.69.169.239:3000/')
    ...
    const savePassword = async () => {
        if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
            ...
            if (form.id) {
                await fetch("http://34.69.169.239:3000/", { method: "DELETE", headers: { "Content-Type": "application/json" },  body: JSON.stringify({ id: form.id }) });
            }
            ...
            // Save to backend
            await fetch("http://34.69.169.239:3000/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newPassword) });
    ...
    const deletePassword = async (id) => {
        ...
        if (c) {
            ...
            await fetch('http://34.69.169.239:3000/', { method: "DELETE", headers: { "content-Type": "application/json" }, body: JSON.stringify({ id }) })
